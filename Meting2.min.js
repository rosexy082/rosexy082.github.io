...
// Object spread utility function
...
class MetingJSElement extends HTMLElement {
  ...
  // Helper to camelize attribute names
  _camelize(a) {
  ...
  _init() {
    ...
    // Set up API URL (use proxy for CORS bypass)
    this.api = this.meta.api || window.meting_api || "https://api.allorigins.win/get?url=https://api.i-meto.com/meting/api?server=:server&type=:type&id=:id&r=:r";
    this.meta.auto && this._parse_link();
  }

  // Parse URL to get server, type, and ID
  _parse_link() {
    let a = [
  ...
  _parse() {
    if (this.meta.url) {
      ...
      return void this._loadPlayer([a]);
    }

    // Modify API request URL
    ...
    fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(a)}`)
      .then(response => response.json())
      .then(data => {
        if (data && data.contents) {
          let musicData = JSON.parse(data.contents);
          this._loadPlayer(musicData);
        }
      })
      .catch(error => console.error("Error loading music data:", error));
  }

  // Load player and display audio
  _loadPlayer(a) {
    let b = { audio: a, mutex: true, lrcType: this.meta.lrcType || 3, storageName: "metingjs" };
    if (a.length) {
